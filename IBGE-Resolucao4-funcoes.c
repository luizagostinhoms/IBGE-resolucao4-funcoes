...
//QUARTA SOLUÇÃO, MELHORANDO A DISTRIBUIÇÃO DE CODIGO 
...
void popularDadosCidade(int num_cidade, cidade cidades[]) {
  ...
  for (int i = 0; i < num_cidade; i++)
    {				//usaremos sempre o num_cidade para varrer a matriz.
      printf ("------------------------------");
...
void descobriCidadeComMaiorNAcidentes(int num_cidade, cidade cidades[]){
    ...
    printf("A cidade com maior indíce de acidentes é: %d", cidadeMaiorAcidente.codigo_cidade);
...
void descobriCidadeComMenorNAcidentes(int num_cidade, cidade cidades[]) {
    ...
    printf("O código da cidade com  menor indíce de acidentes é: %d", cidadeMenorAcidente.codigo_cidade);
